...
function drawPlane(canvas, context){
	...
	if (simulationState == SimulationStateEnum.STOPPED){
		context.drawImage(plane_hint_image, 30, -10, plane_hint_image.width / 20, plane_hint_image.height / 20);	
	}
	context.restore();
...
function drawHUD(context){
	...
	context.fillText("Drop Zone X: " + dropZone.x + "m", 1200, 90);
	...
	if (packageDropped){
		...
		context.fillText("s: " + timeSinceDrop + "s" ,1200,120);
		context.fillText("x_p(s): " + package_x + "m" ,1200,137);
		context.fillText("y_p(s): " + package_y + "m" ,1200,154);		
	}
...
function drawBackground(context){
	...
	for (var i = 0; i < clouds.length; i++){
		context.drawImage(cloud_image, clouds[i].x, clouds[i].y, clouds[i].width, clouds[i].height);
	}
